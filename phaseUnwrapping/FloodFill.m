...
figure; imagesc(immultiply(IM_phase, ~branch_cuts)), colormap(gray), axis square, axis off, title('Select a known true phase reference point');
figure; imagesc(IM_phase), colormap(gray), axis square, axis off, title('Select a known true phase reference point');
uiwait(msgbox('Select a known true phase reference point','Phase reference point','modal'));
[xref,yref] = ginput(1); 
%  xref=132;%for head %130;%100;%for knee %102;%for fat/water phantom %128;
%  yref=223;%for head %240;%270;%for knee %273;%for fat/water phantom %240;
colref=round(xref); rowref=round(yref);
close;
if (branch_cuts(rowref,colref)==1)
    warning('Selected point corresponds to a branch cut.');
end
