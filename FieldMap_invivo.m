...
% GRE5=load('Fieldmaps/meas_MID22_gre_TE10_FID17079_Kspace');
% GRE10=load('Fieldmaps/meas_MID23_gre_TE5_FID17080_Kspace');
% 
...
scalingFactor = 20;
numImgs = 6;
max_box_radius=5;                           %Maximum search box radius (pixels)
...
u_GREFM=(angle(u_data{1})-angle(u_data{2}))/(5*10^-3 * 2*pi) - 2.4; %no scalingFactor?


%M Field Map
% M = EllipticalModel2D(pc0c1, pc90c1, pc180c1, pc270c1);
% GSFM=angle(M)*2/(2*pi*.01);
u_M = EllipticalModel2D(u_data{3},u_data{4},u_data{5},u_data{6});
u_GSFM=angle(u_M)*2/(2*pi*.01);
...
err = abs(u_CGREFM + (u_CGSFM-4));
subplot(1,3,1); imagesc(u_CGREFM,[-50, 50]); title('GRE Field Map');
axis off;
subplot(1,3,2); imagesc(-(u_CGSFM-4),[-50, 50]); title('Elliptical Signal Model Field Map');
axis off;
subplot(1,3,3); imagesc(err/maximum, [0, 1]); title('Error');
axis off;
